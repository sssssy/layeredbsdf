...
//* [fjh] to fix a python undefined symbol error*//
//* [fjh] directly copied from multilayered_bsdfList *//
#include <vector>
#include <ctime>
...
/*!\plugin{RoughDielectric4Hemisphere}{Rough dielectric material}
 * \order{5}
 * \icon{bsdf_RoughDielectric4Hemisphere}
 * \parameters{
 ...
 *         {bsdf_RoughDielectric4Hemisphere_beckmann_0_0_2.jpg}
 *     \rendering{Rough glass (Beckmann, $\alpha=0.1$)}
 *         {bsdf_RoughDielectric4Hemisphere_beckmann_0_1.jpg}
 * }
 *
 ...
 *         \lstref{RoughDielectric4Hemisphere-roughglass})}{bsdf_RoughDielectric4Hemisphere_ggx_0_304.jpg}
 *     \rendering{Textured roughness (\lstref{RoughDielectric4Hemisphere-textured})}
 *         {bsdf_RoughDielectric4Hemisphere_textured.jpg}
 * }
 *
 * \begin{xml}[caption=A material definition for ground glass, label=lst:RoughDielectric4Hemisphere-roughglass]
 * <bsdf type="RoughDielectric4Hemisphere">
 ...
 * \end{xml}
 *
 * \begin{xml}[caption=A texture can be attached to the roughness parameter, label=lst:RoughDielectric4Hemisphere-textured]
 * <bsdf type="RoughDielectric4Hemisphere">
 ...
 * \end{xml}
 */
class RoughDielectric4Hemisphere : public BSDF {
public:
	/// # add by GY
	RoughDielectric4Hemisphere() : BSDF(Properties()) { }

    RoughDielectric4Hemisphere(const Properties &props) : BSDF(props) {
    ...
    RoughDielectric4Hemisphere(Stream *stream, InstanceManager *manager)
     : BSDF(stream, manager) {
	...
	/// # add by GY
	virtual ~RoughDielectric4Hemisphere() {
...
#if 0
	//* [fjh] bsdfList, to return an array of bsdf values on ONE hemisphere *//
	float* bsdfList(int resWi, int resWo, int hemisphere, int _seedIndex, bool highspp) const {

        //* [fjh] rough dielectric doesn't need highspp *//
        Assert(highspp == false);

        int indexSeed = 114514 + _seedIndex;

		//* [fjh] 4 for R+ T+ T- R- *//
		//* [fjh] 7 for wix, wiy, wox, woy, R, G, B *//
		const int arraySize = resWi * resWi * resWo * resWo * 7;
...
#pragma omp parallel for	
		for (int iwi_ = 0; iwi_ < resWi * resWi; iwi_++) //* [fjh] for omp *//
		{
			int jwi = iwi_ % resWi;
			int iwi = (iwi_ - jwi) / resWi;
			int tid = mts_omp_get_thread_num();
#else
		for (int iwi = 0; iwi < resWi; iwi++)
		{
			for (int jwi = 0; jwi < resWi; jwi++)
			{
			int tid = 0;
#endif
				...
				Vector wi = Vector3(
					(iwi + random1) / float(resWi) * 2 - 1, //* [fjh] [0, 1] -> [-1, 1] *//
					(jwi + random2) / float(resWi) * 2 - 1, 
					1.0
				);

				if (wi.x * wi.x + wi.y * wi.y < 1)
				{
					int indexWi = iwi * resWi + jwi;
					wi.z = sqrt(1.0 - wi.x * wi.x - wi.y * wi.y);
					//* [fjh] generate wo *//
					for (int iwo = 0; iwo < resWo; iwo++)
					{
						for (int jwo = 0; jwo < resWo; jwo++)
						{
							//* [fjh] jittering *//
							float random3 = sampler->next1D();
							float random4 = sampler->next1D();
							Vector wo = Vector3(
								(iwo + random3) / float(resWo) * 2 - 1, //* [fjh] [0, 1] -> [-1, 1] *//
								(jwo + random4) / float(resWo) * 2 - 1, 
								1.0
							);

							if (wo.x * wo.x + wo.y * wo.y < 1)
							{
								int indexWo = iwo * resWo + jwo;

								wo.z = (hemisphere == 0 || hemisphere == 2) ? //* [fjh] R+T-: wo is positive *//
									sqrt(1.0 - wo.x * wo.x - wo.y * wo.y) 
									:-sqrt(1.0 - wo.x * wo.x - wo.y * wo.y);
								if (hemisphere >= 2) //* [fjh] T-R-: wi is negative *//
									wi.z = -sqrt(1.0 - wi.x * wi.x - wi.y * wi.y);
								...
								int index = (indexWi * resWo * resWo + indexWo) * 7;
								result[index + 0] = wi.x; //* [fjh] keep the interval of [-1, 1] *//
								result[index + 1] = wi.y;
								result[index + 2] = wo.x;
								result[index + 3] = wo.y;
    ...
    Spectrum eval(const Vector& wi, const Vector& wo, const Intersection& its) const {
        if (Frame::cosTheta(wi) == 0)
            return Spectrum(0.0f);

        /* Determine the type of interaction */
        bool reflect = Frame::cosTheta(wi)
            * Frame::cosTheta(wo) > 0;
        ...
        /* Fresnel factor */
        const Float F = fresnelDielectricExt(dot(wi, H), m_eta);
        ...
        if (reflect) {
            ...
            Float value = F * D * G /
                (4.0f * std::abs(Frame::cosTheta(wi)));

            return m_specularReflectance->eval(its) * value;
        }
        else {
            Float eta = Frame::cosTheta(wi) > 0.0f ? m_eta : m_invEta;
            ...
            Float factor = //(bRec.mode == ERadiance)
                 (Frame::cosTheta(wi) > 0 ? m_invEta : m_eta);// : 1.0f;

            return m_specularTransmittance->eval(its)
	...
	/// # add by GY
	ref<BSDF> clone() {
		ref<RoughDielectric4Hemisphere> bsdf = new RoughDielectric4Hemisphere();

		//bsdf->m_components = m_components;
		//bsdf->m_combinedType = m_combinedType;
		bsdf->m_usesRayDifferentials = m_usesRayDifferentials;
		bsdf->m_ensureEnergyConservation = m_ensureEnergyConservation;

		bsdf->m_type = m_type;
		bsdf->m_specularTransmittance = m_specularTransmittance;
		bsdf->m_specularReflectance = m_specularReflectance;
		bsdf->m_alphaU = m_alphaU;
		bsdf->m_alphaV = m_alphaV;
		bsdf->m_eta = m_eta;
		bsdf->m_invEta = m_invEta;
		bsdf->m_sampleVisible = m_sampleVisible;
    ...
    std::string toString() const {
        ...
        oss << "RoughDielectric4Hemisphere[" << endl
            << "  id = \"" << getID() << "\"," << endl
...
class RoughDielectric4HemisphereShader : public Shader {
public:
    RoughDielectric4HemisphereShader(Renderer *renderer, Float eta) :
...
Shader *RoughDielectric4Hemisphere::createShader(Renderer *renderer) const {
    return new RoughDielectric4HemisphereShader(renderer, m_eta);
}

MTS_IMPLEMENT_CLASS(RoughDielectric4HemisphereShader, false, Shader)
MTS_IMPLEMENT_CLASS_S(RoughDielectric4Hemisphere, false, BSDF)
MTS_EXPORT_PLUGIN(RoughDielectric4Hemisphere, "Rough dielectric BSDF");
MTS_NAMESPACE_END
